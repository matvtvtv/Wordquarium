...
public class WordsFileReader {
    ...
    // Конструктор для первого файла (words_with_length.txt)
    public WordsFileReader(Context context) {
        loadWords(context);
    }


    public WordsFileReader(Context context, boolean isSecondFile) {
        if (isSecondFile) {
            loadWords_2(context);
        }
    }
    ...
    private void loadWords_2(Context context) {
